		...
		if(i >= points.length){
			...
			subinterval_id = setInterval(function(){
				...
				if(j >= queued_markers.length){
					...
					html += "<button id="+queued_markers[j]['s_id']+" onclick='visited(this)'>Посетих го</button><br /><img style='width:200px;height:200px;' src='../images/map_images/"+queued_markers[j]['s_filename']+"' title='"+queued_markers[j]['s_name']+"'/><br />"+queued_markers[j]['source']+"<br />";
					if(queued_markers[j]['wikipedia_link'] != ''){
						html += '<a target="_blank" href="'+queued_markers[j]['wikipedia_link']+'">Източник</a>';
					}
					html += "</div>";
...
function visited(button_ele){
		...
		request.done(function( response ) {
		   ...
		   if(response==2){
			  $(button_ele).after('<b>Вие вече сте посещавали този обект. <br />Благодаря ви за повторното идване.</b>');
			  $(button_ele).hide();
		   }
		});
