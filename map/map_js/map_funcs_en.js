...
function createMarker(location, text, title, is_national){
	...
	if(is_national==1){
		marker.setIcon('http://posetime.noit.eu/images/marker_blue.png');
	}
...
function calcRoute() {
	...
	if(start.length > 0 && end.length > 0 && radius.length > 0 && radius > 0 && radius < 110){
		var request = {
			origin: start,
			destination: end,
			travelMode: 'DRIVING'
		...
		directionsService.route(request, function(result, status) {
			if (status == 'OK') {
				...
				$('#show_circles_btn').notify("We could not find one of the points, please try to write the points differently.");
			}else{
				$('#show_circles_btn').notify("An unexpected error occurred.");
			}
		});
	}else{
		$('#show_circles_btn').notify("Please,enter values.", 'info');
	}
}
		...
		if(i >= points.length){
			...
			subinterval_id = setInterval(function(){
				...
				if(j >= queued_markers.length){
					...
					html += "<button id="+queued_markers[j]['s_id']+" onclick='visited(this)'>I visited him</button><br /><img style='width:200px;height:200px;' src='../images/map_images/"+queued_markers[j]['s_filename']+"' title='"+queued_markers[j]['s_name']+"'/><br />"+queued_markers[j]['source']+"<br />";
					if(queued_markers[j]['wikipedia_link'] != ''){
						html += '<a target="_blank" href="'+queued_markers[j]['wikipedia_link']+'">Source</a>';
					}
					html += "</div>";
					
					marker = createMarker({lat: queued_markers[j]['s_lat'], lng: queued_markers[j]['s_lng']}, html, queued_markers[j]['s_name'], queued_markers[j]['is_national']);
...
function visited(button_ele){
		...
		request.done(function( response ) {
		   if(response==1){
			  $(button_ele).after('<b>Visited</b>');
			  $(button_ele).hide();
		   }
		   if(response==2){
			  $(button_ele).after('<b>You have already visited this object. <br /> Thank you for coming back.</b>');
			  $(button_ele).hide();
		   }
		});
